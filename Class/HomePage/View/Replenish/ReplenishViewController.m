...
#import "ChangeBtn.h"
#import "RepDetailsViewController.h"//详情
#import "RepSouchViewController.h"//搜索

#import "ReplenishStCell.h"//进货信息
#import "ReplenishStModel.h"

#import "AllStoreCell.h"//popView显示数据
#import "AllStoreModel.h"

@interface ReplenishViewController ()<UITableViewDelegate,UITableViewDataSource,UISearchBarDelegate,MBProgressHUDDelegate>

{
    ...
    //NSMutableArray *_StoreDataArray;//店铺的数据
...
- (void)viewDidLoad {
    ...
    [self addNavLabel:CGRectMake(ScreenWidth/2.737, 25, 100, 30) font:[UIFont systemFontOfSize:20] textColor:[UIColor whiteColor] textAlignment:NSTextAlignmentCenter text:@"进货信息"];
...
-(void)createTableview
{
    ...
    _tableView= [[UITableView alloc]initWithFrame:CGRectMake(0, 44, ScreenWidth, ScreenHeight) style:UITableViewStyleGrouped];
...
-(void)createScan
{
    ...
    //下面的线条
    UIImageView *LowLine = [MyUtil createIamgeViewFrame:CGRectMake(0, 46, ScreenWidth, 0.5) imageName:@"375x1@2x"];
...
-(void)openSouchAction:(UIButton *)sender
{
    ...
    RepSouchViewController *stockShCtrl = [[RepSouchViewController alloc]init];
...
-(void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (tableView == _tableView) {
        ...
        RepDetailsViewController *SalesCtrl = [[RepDetailsViewController alloc]init];
        
        ReplenishStModel *model = self.dataArray[indexPath.row];
        
        SalesCtrl.storeIncomeNo =model.GUID;
    ...
    }else if (tableView == _popViewTableview)
    {
        ...
        for (_chgtn in arr) {
            ...
            if (_chgtn.tag == 10) {
                ...
                str = [NSString stringWithFormat:@"%@",bgyl];
                
                
                //点击cell 修改 btn 文字 并下载数据
                ReplenishStModel *cellModel = self.StoreDataArray[indexPath.row];
...
-(UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (tableView == _tableView) {
        ...
        ReplenishStModel *cellModel = self.dataArray[indexPath.row];
...
-(void)download
{
    ...
    NSString *str = [NSString stringWithFormat:@GetStoreIncomeListByPageUrl];
    ...
    [AFHTTPClientV2 requestWithBaseURLStr:str
                                  ...
                                  success:^(AFHTTPClientV2 *request, id responseObject)
     {
         ...
         //保存下载的数据用于缓存
         [CacheManager saveCacheWithObject:responseObject ForURLKey:@"3" AndType:CacheTypeQuestion];
         ...
         if ([dict isKindOfClass:[NSDictionary class]]) {
             ...
             if ([dic isKindOfClass:[NSDictionary class]]) {
                 ...
                 for (NSDictionary *dict in arr1) {
                     ...
                     ReplenishStModel *model = [[[ReplenishStModel alloc]init]initWithDictionary:sssDict];
...
-(void)Caches
{
    if ([CacheManager readCacheWithURLKey:@"3" andType:CacheTypeQuestion]) {
        
        id result = [CacheManager readCacheWithURLKey:@"3" andType:CacheTypeQuestion];
        ...
        switch (status) {
            ...
            case AFNetworkReachabilityStatusNotReachable:
                ...
                if (self.dataArray != nil) {
                    ...
                    if ([CacheManager readCacheWithURLKey:@"3" andType:CacheTypeQuestion]!= nil) {
